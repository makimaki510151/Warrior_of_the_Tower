(function (root) {
  ...
  function renderHelpBody() {
    const stackPct = Math.round((W.STACK_GAIN_BONUS || 0.3) * 100);
    const compare = (W.COMPARE_RULES_HELP || []).map((line) => `<li>${esc(line)}</li>`).join("");
    const sections = [
      {
        heading: "このゲームでやること",
        ...
        items: [
          ...
          "負けても手順の組み直しができる。諦めると技は消えるが、最高記録は残る。",
        ],
      },
      {
        heading: "技の獲得",
        lead: `全${W.SKILLS.length}種から、毎回5枚の候補が出る。そのうち1つだけ選ぶ。`,
        items: [
          "同じ技を重ねると効果は伸びる。",
      ...
      },
      {
        heading: "戦闘の見方",
        items: [
          "ログの行動番号は、あなたと敵で別々に数える。",
          "倍速は 1／2／4／8／16。結果へスキップもできる。",
          "毒や終焔など遅延するダメージは、付与した時点の攻撃力・与ダメ補正を使う。防御・軽減はダメージ発生時の値。",
          `お互いの手番を合わせて${W.MAX_ACTIONS || 300}手番動いても決着がつかない場合は、プレイヤーの敗北になる。`,
        ],
      },
      {
        heading: "比較の言葉",
        lead: "条件や説明で使う比較は、次の意味です。",
        listHtml: compare ? `<ul class="help-list">${compare}</ul>` : "",
      },
      {
        heading: "敵と育成",
        items: [
  ...
  function modal() {
    ...
    if (ui.help) {
      return `
        <div class="modal" role="dialog" aria-modal="true" aria-label="遊び方">
          <div class="modal-card modal-card-wide help-card">
